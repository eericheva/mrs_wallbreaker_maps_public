# How the map is filled and built: Scheming & Deceptive Alignment
...
Build anchor (as_of): 2026-07. Papers in catalog: 70; points on the map: 107; closed by ≥1 paper: 100. Future-work requests: 54. Citation "giants": 6 (Tukey threshold 112 citations, absolute floor 100).
...
- The two horizontal lines are the "giant" thresholds: the absolute floor of 100 citations (red dashed) and the relative Tukey threshold Q3+1.5·IQR = 112 citations for THIS catalog (orange dashed). Above either one is a giant, which must survive a rebuild.
...
## How the weighted catalog lands on the tensor (RQ × SETTING)

![How the weighted catalog lands on the tensor (RQ × SETTING)](fig_tensor_heatmaps.png)

- What the figure shows: five grid heatmaps. Rows are RQ directions (the root axis), columns are values of the "SETTING" axis (the map itself chose it as the one that most sharply separates demand-gaps). Each grid cell aggregates all tensor points with these two coordinates (averaging over the other axes), so the whole N-dimensional tensor projects onto the plane.
