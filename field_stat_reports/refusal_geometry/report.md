...
- Closest to the archetype **Broad shallow** (sampled everywhere, re-checked almost nowhere — one paper per map point); Euclidean distance over the 6 composite axes 0.34 (0 — exactly at the type's center).
...
- **Areas (map axes).**
  - most asked-for area "non-unique-direction: no" (total demand 207 request papers)
